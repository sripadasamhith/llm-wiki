...
> **This repo is a template.** If `wiki/overview.md` still contains `<!-- TEMPLATE -->`,
> nothing has been ingested yet. See [First run](#first-run) at the bottom.

---

## Project

<!-- Fill this in on first run. Keep it to a few lines. It is the highest-leverage
     context in this file — everything you write should serve this. -->

**Subject:** _(what this wiki is about)_
**Mode:** _(technical | research | mixed — see Domain profiles)_
**Purpose:** _(what I want to be able to do with this wiki in 3 months)_
...
**Yours (the agent):** everything else. You write and maintain 100% of `wiki/`. I read it.
...
|---|---|---|---|
| Raw sources | `raw/` | Me (and you, when saving fetched content) | **Immutable** once filed. Never edit or delete. |
| The wiki | `wiki/` | You, exclusively | Continuously revised |
| The schema | `AGENTS.md` (this file) | Both, by agreement | Rarely |

`raw/` is the source of truth. If the wiki and a raw source disagree, the source wins and
...
templates/                page skeletons — copy these, don't link to them
docs/                     setup notes for humans (Obsidian, etc.)
```

Create new top-level `wiki/` subdirectories only when a category has 5+ pages that don't
...
Extra fields per page type are listed in `templates/`.

---

## The two navigation files

**`wiki/index.md` — content-oriented.** A catalog of every page in the wiki, grouped by
...
**`wiki/log.md` — chronological.** Append-only. Never rewrite past entries. Every entry
...
so that `grep "^## \[" wiki/log.md | tail -20` gives a usable recent history. Under each
...
Trigger: I drop something in `raw/` and say "ingest this," or hand you a URL.

1. **Read the source completely** before writing anything. If it's a markdown file with
   inline images, read the text first, then view the referenced images in `raw/assets/`
...
3. **Write `wiki/sources/<slug>.md`** — the summary page. Faithful to the source, in your
...
4. **Propagate.** This is the part that matters and the part that's easy to shortchange.
   Walk `index.md` and update every page the source touches:
   ...
   - `wiki/overview.md` → revise if the synthesis actually moved. Don't churn it if not.
5. **Update `index.md`** with every new and changed page.
6. **Append to `log.md`**, listing every page touched.
...
**Filing a raw source:** name it `raw/YYYY-MM-DD-short-slug.ext`. If I give you a URL,
fetch it and save the markdown into `raw/` before summarizing, so the source is preserved
even if the page dies. Never edit a file in `raw/` after filing it.

### 2. Query

Trigger: I ask a question.

1. Read `index.md`. Pick the relevant pages. Read them fully.
2. Only if the wiki is thin on the topic, fall back to reading `raw/` directly — and if
...
- **Index drift** — pages missing from `index.md`, or index entries pointing nowhere
...
- Let `index.md` drift out of sync with the actual files
...
- Delete anything from `raw/`, or rewrite history in `log.md`
...
When this template is freshly cloned and I ask you to set it up:

1. Interview me briefly — subject, mode, purpose, what sources I have. Fill in the
   **Project** section above.
2. Prune the domain profile I'm not using, if I'm clearly one or the other.
3. Reset `wiki/overview.md`, `wiki/index.md`, and `wiki/log.md` to empty starting states
   for this project (remove the `<!-- TEMPLATE -->` markers).
4. Delete this `First run` section from `AGENTS.md`.
5. Ingest the first source together, walking each step out loud so we can tune the
   workflow before it hardens.
